...
import com.arta.lib.widget.listener.OnTreeItemViewDecorateListener;
 ...
 * @param <T2> 子视图数据实体
 */
public class BaseEntitySelectedTreeViewAdapter<T1, T2> extends
		BaseEntityTreeViewAdapter<T1, T2> {
	...
	private OnTreeItemViewDecorateListener decorateListener;
	
	private boolean singleGroupExpandEnable = false;
	
	public BaseEntitySelectedTreeViewAdapter(ExpandableListView expandListView,
	...
	public BaseEntitySelectedTreeViewAdapter(ExpandableListView expandListView,
			List<T1> groupEntityList, List<List<T2>> childEntityList,
			BaseAdapterEntityTreeViewManage<T1, T2> viewManage,OnTreeItemViewDecorateListener decorateListener,
			int[] selectGroupDrawable, int[] selectChildDrawable) {
	...
	public void updateHeader(View header, int groupPosition, int childPosition,
			int alpha) {
		super.updateHeader(header, groupPosition, childPosition, alpha);
		if(selectedGroud == groupPosition && selectedChild == -1){
			if(header instanceof ImageView){
				((ImageView)header).setImageResource(selectGroupDrawable[0]);
			}
			else{
				header.setBackgroundResource(selectGroupDrawable[0]);
			}
		}
		else{
			if(header instanceof ImageView){
				((ImageView)header).setImageResource(selectGroupDrawable[1]);
			}
			else{
				header.setBackgroundResource(selectGroupDrawable[1]);
			}
		}
	}

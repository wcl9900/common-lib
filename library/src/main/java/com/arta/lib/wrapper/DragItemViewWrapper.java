...
import com.arta.lib.adapter.treeview.BaseEntityTreeViewAdapter;
...
/**
 * 列表视图中可拖动子视图的包装类, 可支持任何基于{@link AbsListView}的列表视图（包括ExpandListView）；当使用普通的一级列表视图时使用{@link OnDragItemListener}监听器;
 * 当使用基于ExpandListView的伸缩列表视图时需使用{@link OnDragExpandItemListener}监听器,数据适配器必须使用{@link BaseEntityTreeViewAdapter}。
 * @author 王春龙
 *
 */
public class DragItemViewWrapper implements OnTouchListener{
	...
	/**
	 * ExpandListView拖动视图监听器
	 * @author 王春龙
	 *
	 */
	public interface OnDragExpandItemListener{
		/**
		 * 拖动子视图开始
		 * @param absListView
		 * @param itemView
		 ...
		 * @param childPosition,
		 */
		public void onDragItemStart(AbsListView absListView, View itemView, boolean isGroup, int groupPosition, int childPosition, MotionEvent event);
		/**
		 * 拖动子视图静止
		 * @param absListView
		 * @param itemView
		 ...
		 * @param childPosition,
		 */
		public void onDragItemStatic(AbsListView absListView, View itemView, boolean isGroup, int groupPosition, int childPosition, MotionEvent event);
		/**
		 * 拖动子视图结束
		 * @param absListView
		 * @param itemView
		 ...
		 * @param childPosition,
		 */
		public void onDragItemEnd(AbsListView absListView, View itemView, boolean isGroup, int groupPosition, int childPosition, MotionEvent event);
	}
	...
	private OnDragExpandItemListener dragExpandListener;
	...
	private void dragStatic(MotionEvent event, float speed) {
		...
		if(speed <= action_static_speed && action_static_enable){
		    ...
		    if(dragExpandListener != null && absListView instanceof ExpandableListView){
		    	if(selectItemView.getTag(AdapterConstant.TAG_KEY) instanceof ViewHolderGroup && dragGroupEnable){
		    		ViewHolderGroup vhg = (ViewHolderGroup) selectItemView.getTag(AdapterConstant.TAG_KEY);
			    	dragExpandListener.onDragItemStatic(absListView, selectItemView, true, vhg.groupPosition, -1, event);
		    	}
		    	else if(selectItemView.getTag(AdapterConstant.TAG_KEY) instanceof ViewHolderChild){
		    		ViewHolderChild vhc = (ViewHolderChild) selectItemView.getTag(AdapterConstant.TAG_KEY);
			    	dragExpandListener.onDragItemStatic(absListView, selectItemView, false, vhc.groupPosition, vhc.childPosition, event);
		    	}
		    }
	 ...
	 * @param listener
	 */
	public void setOnDragExpandListener(OnDragExpandItemListener listener){
		this.dragExpandListener = listener;
	}
	...
	private void endDrag(MotionEvent event) {
	    ...
	    if(dragExpandListener != null && absListView instanceof ExpandableListView){
	    	if(selectItemView.getTag(AdapterConstant.TAG_KEY) instanceof ViewHolderGroup && dragGroupEnable){
	    		ViewHolderGroup vhg = (ViewHolderGroup) selectItemView.getTag(AdapterConstant.TAG_KEY);
		    	dragExpandListener.onDragItemEnd(absListView, selectItemView, true, vhg.groupPosition, -1, event);
	    	}
	    	else if(selectItemView.getTag(AdapterConstant.TAG_KEY) instanceof ViewHolderChild){
	    		ViewHolderChild vhc = (ViewHolderChild) selectItemView.getTag(AdapterConstant.TAG_KEY);
		    	dragExpandListener.onDragItemEnd(absListView, selectItemView, false, vhc.groupPosition, vhc.childPosition, event);
	    	}
	    }
	...
	private boolean startDrag(MotionEvent event) {
		...
		if(pointToPosition != AdapterView.INVALID_POSITION){
			...
			if(absListView instanceof ExpandableListView && !dragGroupEnable && selectItemView.getTag(AdapterConstant.TAG_KEY) instanceof ViewHolderGroup){
				return false;
			}
		    ...
		    else if(dragExpandListener != null && absListView instanceof ExpandableListView){
		    	if(selectItemView.getTag(AdapterConstant.TAG_KEY) instanceof ViewHolderGroup && dragGroupEnable){
		    		ViewHolderGroup vhg = (ViewHolderGroup) selectItemView.getTag(AdapterConstant.TAG_KEY);
			    	dragExpandListener.onDragItemStart(absListView, selectItemView, true, vhg.groupPosition, -1, event);
		    	}
		    	else if(selectItemView.getTag(AdapterConstant.TAG_KEY) instanceof ViewHolderChild){
		    		ViewHolderChild vhc = (ViewHolderChild) selectItemView.getTag(AdapterConstant.TAG_KEY);
			    	dragExpandListener.onDragItemStart(absListView, selectItemView, false, vhc.groupPosition, vhc.childPosition, event);
		    	}
		    }

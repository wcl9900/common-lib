...
package com.arta.lib.widget.flipview;

import com.arta.lib.widget.flipview.utils.AphidLog;
...
import static com.arta.lib.widget.flipview.FlipRenderer.checkError;
import static com.arta.lib.widget.flipview.utils.TextureUtils.d2r;
import static com.arta.lib.widget.flipview.utils.TextureUtils.isValidTexture;
import static com.arta.lib.widget.flipview.utils.TextureUtils.toFloatBuffer;
import static com.arta.lib.widget.flipview.utils.TextureUtils.toShortBuffer;
...
public class Card {
  ...
  public void draw(GL10 gl) {
    ...
    if (angle > 0) {
      ...
      if (axis == AXIS_TOP) {
        if (orientationVertical) {
          h = (cardVertices[Y_00] - cardVertices[Y_01]) * (float) (1.0f - Math.cos(d2r(angle)));
          ...
          w = (cardVertices[X_10] - cardVertices[X_00]) * (float)(1.0f - Math.cos(d2r(angle)));
        ...
        if (orientationVertical) {
          h = (cardVertices[Y_00] - cardVertices[Y_01]) * (float)(1f - Math.cos(d2r(angle)));
          z = (cardVertices[Y_00] - cardVertices[Y_01]) * (float)Math.sin(d2r(angle));
          shadowVertices = new float[]{
          ...
          w = (cardVertices[X_10] - cardVertices[X_00]) * (float)(1f - Math.cos(d2r(angle)));
          z = (cardVertices[X_10] - cardVertices[X_00]) * (float)Math.sin(d2r(angle));
          shadowVertices = new float[]{
              cardVertices[X_00], cardVertices[Y_00], 0f,
              cardVertices[X_01], cardVertices[Y_01], 0f,

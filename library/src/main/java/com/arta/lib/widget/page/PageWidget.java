package com.arta.lib.widget.page;
...
public class PageWidget extends View {
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		if (event.getAction() == MotionEvent.ACTION_DOWN) {
			mCanvas.drawColor(0xFFAAAAAA);
			if(isopen == false){
				mTouch.x = mWidth-50;//event.getX();
				mTouch.y = 60;//event.getY();
				isopen = true;
			}else{
				mTouch.x = mWidth;//event.getX();
				mTouch.y = 0;//event.getY();
				isopen = false;
			}
		...
		if (event.getAction() == MotionEvent.ACTION_UP) {
			//			mCanvas.drawColor(0xFFAAAAAA);
			//			if(!isopen){
			//				mTouch.x = 395;//event.getX();
			//				mTouch.y = 731;//event.getY();
			//				isopen = true;
			//				}else{
			//					mTouch.x = 400;//event.getX();
			//					mTouch.y = 800;//event.getY();
			//					isopen = false;
			//				}
			//			this.postInvalidate();
		}
	...
	public void drawCurrentPageShadow(Canvas canvas) {
		...
		// 注：拖拽顶点的阴影，定位有问题，待修改~~
		double d1 = Math.atan2(mBezierControl1.y - mTouch.y, mTouch.x
				- mBezierControl1.x);

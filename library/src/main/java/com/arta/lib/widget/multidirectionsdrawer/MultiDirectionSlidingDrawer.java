...
package com.arta.lib.widget.multidirectionsdrawer;
...
public class MultiDirectionSlidingDrawer extends ViewGroup {
	 ...
	 * @param value
	 */
	public void setVeloctiy(float value){
		this.velocity = value;
		final float density = getResources().getDisplayMetrics().density;
		mMaximumAcceleration = (int)( MAXIMUM_ACCELERATION * density * velocity + 0.5f );
		if(mInvert){
	...
	public MultiDirectionSlidingDrawer( Context context, AttributeSet attrs, int defStyle )
	{
		...
		int handleId = a.getResourceId( R.styleable.MultiDirectionSlidingDrawer_msdHandle, 0 );
		if ( handleId == 0 ) { throw new IllegalArgumentException( "The handle attribute is required and must refer "
				+ "to a valid child." ); }
		
		int contentId = a.getResourceId( R.styleable.MultiDirectionSlidingDrawer_msdContent, 0 );
